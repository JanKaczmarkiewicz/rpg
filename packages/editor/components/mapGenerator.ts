...
const createEmpty2DArray = (size: number): undefined[][] =>
    Array(size)
        .fill(undefined)
...
function randomMove(current: Coordinate): Coordinate | null {
    ...
    for (const direction of shuffle([...DIRECTIONS])) {
        const moveCandidate: Coordinate = {
            x: direction.x + current.x,
            y: direction.y + current.y,
        };
...
export default function mapGenerator() {
    ...
    const startCoordinate: Coordinate = {
        x: random(config.MAP_SIZE),
        y: random(config.MAP_SIZE),
    };
